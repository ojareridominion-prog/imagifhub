...
async def like_media(media_id: int, payload: dict = Body(...)):
    ...
    try:
        # Use upsert to handle the unique constraint (user_id + media_id)
        # This acts as the 'Like' and prevents duplicates
        supabase.table('likes').upsert({
            "user_id": user_id,
            "media_id": media_id
        }).execute()
        
        # Count total likes for this image from the dedicated likes table
